...
当前 macOS 原型已接通屏幕录制权限、主显示器截图、普通 WebP 与缩略图原子写入、SQLite 时间线和启动索引恢复。macOS 与 Windows 的锁屏、休眠/唤醒和空闲监听以及动态托盘控制已接入同一 Rust 状态机；个人 SFTP 配置、上传队列与手动选择界面已进入第一期实现。Windows 捕获、系统事件与托盘真机验收仍待完成。
