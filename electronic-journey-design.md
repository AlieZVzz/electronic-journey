...
- 不自动上传新截图，不提供后台静默同步开关。
- 第一期不提供多服务器、多配置、定时上传、双向同步或远端删除。
...
- 权限必须由系统流程明确授予。
- 默认间隔 5 分钟；停止、暂停、锁屏、休眠和权限变化应进入明确状态。
...
保存 UUID、设备和显示器标识、`captured_at_utc`、采集时区、本地相对路径、缩略图相对路径、文件大小、SHA-256 和缩略图状态。时间线排序只使用数据库时间。

### 6.2 `remote_profiles`

第一期只使用固定标识 `primary`，保存名称、主机、端口、用户名、私钥路径、固定主机指纹、远程根目录、`has_passphrase` 和时间戳。`has_passphrase` 只表示系统钥匙串中应存在口令。

### 6.3 `upload_batches`

状态包括 `pending`、`uploading`、`completed`、`partial_failed` 和 `cancelled`，并保存总项目数、总字节数、完成数和失败数。崩溃恢复时，遗留的 `uploading` 不得自动继续向网络发送。
...
`YYYY/MM/DD/YYYYMMDDTHHMMSSmmmZ_<capture_uuid>.webp`

相对路径来自 SQLite 中的采集时间和 UUID，不接受前端传入文件名。客户端只在根目录下创建日期子目录。
...
- 默认无外发，网络操作必须来自用户读取指纹、测试连接或确认上传。
...
Rust 和 SQLite 测试覆盖输入验证、路径逃逸、迁移、批次上限、确定性文件名、上传状态与删除互锁。前端检查覆盖浏览器 fallback、全选范围、上传确认、重复提交、结果文案和口令不回显。
...
这些候选均不得引入自动上传或让客户端感知远端 Hermes/LLM。
