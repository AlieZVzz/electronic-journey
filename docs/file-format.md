...
- SQLite 同时保存完整像素摘要和可空的版本化稳定内容摘要。稳定摘要只在受验证的系统边缘栏区域可用时生成；它不改变 WebP 字节。
...
- 历史恢复扫描无法可靠重建当时的系统栏几何，因此只回填完整像素摘要，不回填稳定内容摘要或比较策略。
- SFTP 上传始终使用经过大小、SHA-256 和解码校验的本地原图，不上传缩略图。
- 远端日期目录和文件名时间由 SQLite 的 UTC 采集时间转换为北京时间（固定 UTC+8）后生成，文件名包含 `+0800` 偏移量和截图 UUID，不接受前端文件名。
